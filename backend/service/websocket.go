	...
	if channelID == client.ChannelID {
		response = map[string]interface{}{
			...
			"data": map[string]interface{}{
				...
				"message":    "have joined this channel",
			},
		}
	} else {
		client.ChannelID = channelID
		// 将用户添加到频道的客户端列表
		addClient(client.ChannelID, client)
		log.Printf("添加用户:%s到client:%s\n", client.UserID, client.ChannelID)
		response = map[string]interface{}{
			"type": "join_channel_ack",
			"data": map[string]interface{}{
				"channel_id": channelID,
...
func (client *Client) handleSendMessage(data SendMessage) {
	...
	broadcastMessage(client, messageBytes)
...
func addClient(channelID string, client *Client) {
	mu.Lock()
	defer mu.Unlock()
